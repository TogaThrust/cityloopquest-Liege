...
import {
  ...
  ensureReleaseNotesBodyText,
  formatReleaseNotesBody,
  removeEmptyReleaseNoteEntries,
  saveBuildBaseline,
...
async function bumpVersion() {
  ...
  const changelogBody = ensureReleaseNotesBodyText(
    formatReleaseNotesBody(buildReleaseNotesLines(ROOT)),
  );
  ...
  const header =
    "CityLoop Quest Liège - Release notes\r\n" +
    `Version ${newVersion}\r\n` +
    "----------------------------------------\r\n" +
    changelogBody +
    "\r\n";
  ...
  if (await exists(RELEASE_NOTES_PATH)) {
    previousNotes = removeEmptyReleaseNoteEntries(
      await readFile(RELEASE_NOTES_PATH, "utf8"),
    );
  }
