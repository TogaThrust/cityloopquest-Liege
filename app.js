...
let quizOverlayStylesInjected = false;

function ensureQuizOverlayStyles() {
  if (quizOverlayStylesInjected) return;
  quizOverlayStylesInjected = true;
  const style = document.createElement('style');
  style.id = 'quiz-overlay-runtime-styles';
  style.textContent = [
    '#quiz-question-overlay .quiz-question-answers{display:flex!important;flex-direction:column!important;gap:10px!important;overflow:visible!important;max-height:none!important;}',
    '#quiz-question-overlay .quiz-answer-btn{display:block!important;width:100%!important;max-width:100%!important;height:auto!important;min-height:0!important;max-height:none!important;overflow:visible!important;padding:12px 14px!important;white-space:normal!important;word-break:break-word!important;overflow-wrap:anywhere!important;text-overflow:clip!important;-webkit-line-clamp:unset!important;line-clamp:unset!important;}',
    '#quiz-question-overlay .quiz-answer-btn__label{display:block!important;white-space:normal!important;word-break:break-word!important;overflow:visible!important;}',
  ].join('');
...
function showQuizQuestionPopup({ title, question, options, stopLabel, onStop, onOptionClick }) {
  ...
  (options || []).forEach((opt, idx) => {
    ...
    optionBtn.className = 'quiz-answer-btn';
    optionBtn.setAttribute('role', 'button');
    optionBtn.tabIndex = 0;
    const label = document.createElement('span');
    label.className = 'quiz-answer-btn__label';
    ...
    optionBtn.addEventListener('click', activate);
    ...
    answersWrap.appendChild(optionBtn);
  });
...
/** Après activation du quiz : avancer une fois le quiz du point quitté terminé. */
let quizPendingAdvanceCallback = null;
...
function getAudioHeardIndices() {
    try {
        const saved = localStorage.getItem('liege_audio_heard_indices');
        if (saved) {
            const parsed = JSON.parse(saved);
            if (Array.isArray(parsed)) return parsed.map(Number).filter(n => !Number.isNaN(n));
        }
    } catch (_) { /* ignore */ }
    const legacy = localStorage.getItem('liege_audio_clicked_index');
...
function getAudioCompletedIndices() {
    try {
        const saved = localStorage.getItem('liege_audio_completed_indices');
        if (saved) {
            const parsed = JSON.parse(saved);
...
function markAudioHeardForIndex(idx) {
    ...
    const arr = [...set];
    try {
        localStorage.setItem('liege_audio_heard_indices', JSON.stringify(arr));
        localStorage.setItem('liege_audio_clicked_index', String(idx));
...
function handlePoiDescriptionAudioEnded(idx) {
    ...
    if (quizResumeAwaitingAudio && Number(idx) === Number(currentIndex)) {
        quizResumeAwaitingAudio = false;
        const pendingAdvance = quizPendingAdvanceCallback;
        quizPendingAdvanceCallback = null;
        setTimeout(() => {
            showQuizForPointIndex(idx, pendingAdvance || (() => {}), { force: true });
        }, 300);
    }
}
...
function clearPoiTourMuseumState() {
    ...
    if (typeof hidePointImageDisplay === 'function') hidePointImageDisplay();
}

function museumForceTargetStorageKey() {
    const prefix = (window.CLQ_CITY && window.CLQ_CITY.storagePrefix) || 'liege';
...
function canStartQuizAtPointIndex(idx) {
  if (isTourFreeRoam()) return true;
  return getAudioHeardIndices().includes(Number(idx));
}

function canStartQuizAtCurrentPoint() {
  return canStartQuizAtPointIndex(currentIndex);
}
...
let completedQuizQuestions = (() => {
    const saved = localStorage.getItem("liege_completedQuizQuestions");
...
const MONS_LAST_KNOWN_POSITION_KEY = 'liege_lastKnownPosition';
...
function resetTourProgressForNewCircuit() {
  currentIndex = 0;
  localStorage.setItem('liege_currentIndex', '0');
  localStorage.removeItem('tourCompleted');
  localStorage.removeItem('liege_audio_heard_indices');
  localStorage.removeItem('liege_audio_clicked_index');
  localStorage.removeItem('liege_audio_completed_indices');
  localStorage.removeItem('liege_quizEnabled');
  localStorage.removeItem('liege_completedQuizQuestions');
  quizEnabled = false;
  quizResumeAwaitingAudio = false;
  quizPendingAdvanceCallback = null;
...
/** Variante fichier image Liège : espaces   underscores, casse conservée. */
...
function snapshotTourStateForExternalPage() {
    localStorage.setItem('liege_currentIndex', String(currentIndex));
    localStorage.setItem('liege_score', String(score));
    try {
...
function bootstrapMainTourDisplay() {
    if (isMuseumGuidanceActive()) return;
    updatePointPhotoForCurrentIndex();
    const dest = getRouteDestination();
    const display = document.getElementById("location-name");
...
function advanceToNextPoint() {
    ...
    if (currentIndex === filteredLocations.length - 2) {
        ...
        const visitedPoints = JSON.parse(localStorage.getItem('liege_visitedPoints') || '[]');
        if (!visitedPoints.includes(currentIndex)) {
            visitedPoints.push(currentIndex);
            localStorage.setItem('liege_visitedPoints', JSON.stringify(visitedPoints));
        }
        ...
        steps = [];
        currentStepIndex = 0;
        localStorage.setItem("liege_currentIndex", currentIndex);
        localStorage.setItem("liege_score", score);
...
function setQuizEnabledRuntime(enabled) {
  quizEnabled = enabled;
  localStorage.setItem('liege_quizEnabled', enabled ? 'true' : 'false');
  if (enabled) localStorage.removeItem('liege_quizStopped');
  updateQuizFooterButton();
...
function stopQuizPermanently() {
  ...
  localStorage.setItem('liege_quizStopped', 'true');
    ...
    buttons: [
      ...
      },
      {
        ...
        onClick: () => {
          ...
          quizResumeAwaitingAudio = false;
          showQuizForPointIndex(currentIndex, () => {}, { force: true });
        }
      }
    ]
  });
...
function showQuizPrompt(callback) {
  if (localStorage.getItem("liege_quizEnabled") !== null) {
    callback(localStorage.getItem("liege_quizEnabled") === "true");
    return;
  }
  const overlay = document.createElement('div');
  ...
  btnYes.style.cursor = 'pointer';
  btnYes.addEventListener('click', () => {
    localStorage.setItem("liege_quizEnabled", "true");
    quizEnabled = true;
    completedQuizQuestions = {};
    localStorage.removeItem('liege_completedQuizQuestions');
    localStorage.removeItem('liege_quizStopped');
    document.body.removeChild(overlay);
    updateQuizFooterButton();
    callback(true);
  });
  ...
  btnNo.style.cursor = 'pointer';
  btnNo.addEventListener('click', () => {
    localStorage.setItem("liege_quizEnabled", "false");
    quizEnabled = false;
    document.body.removeChild(overlay);
    updateQuizFooterButton();
    callback(false);
...
async function ensureQuizTranslationsLoaded() {
  if (window.quizTranslations && Object.keys(window.quizTranslations).length > 0) return;
  try {
    await loadQuizTranslations();
  } catch (_) { /* ignore */ }
}

function resolveQuizQuestionsForPoint(pointName) {
  ...
  let questions = langData[name];
  if (!questions || !questions.length) {
    questions = window.quizData ? window.quizData[name] : null;
  }
  return questions && questions.length ? questions : null;
}
...
function showQuizForPointIndex(index, callback, options = {}) {
  ...
  if (!force && !quizEnabled) {
    runAfter();
    return;
  }

  if (!canStartQuizAtPointIndex(pointIndex)) {
    quizPendingAdvanceCallback = runAfter;
    quizResumeAwaitingAudio = true;
    showQuizListenBeforePopup();
    return;
  }
  ...
  void (async () => {
    if (!window.translationManager) {
      setTimeout(() => showQuizForPointIndex(pointIndex, runAfter, options), 100);
      return;
    }

    await ensureQuizTranslationsLoaded();

    const currentLocation = getLocationAtIndex(pointIndex);
    if (!currentLocation?.name) {
      runAfter();
      return;
    }
    const pointName = currentLocation.name;
    const questions = resolveQuizQuestionsForPoint(pointName);
    if (!questions) {
      runAfter();
      return;
    }
    if (!force && completedQuizQuestions[pointName]) {
      runAfter();
      return;
    }
    if (force && completedQuizQuestions[pointName]) {
      delete completedQuizQuestions[pointName];
      localStorage.setItem('liege_completedQuizQuestions', JSON.stringify(completedQuizQuestions));
    }
    ...
    function getQuizCorrectIconSrc() {
      return (window.CLQ_CITY && window.CLQ_CITY.quizCorrectIcon) || 'images/quiz-correct.png';
    }
    ...
    function afterAnswerFeedback(isGood, fields) {
      ...
      feedbackImg.src = isGood ? getQuizCorrectIconSrc() : 'images/incorrect.png';
...
function showQuizForCurrentPoint(callback, options = {}) {
  showQuizForPointIndex(currentIndex, callback, options);
}
...
function initializeMainLogic() {
    ...
    // Restaurer l'état de l'application depuis le localStorage (sauf après nouveau parcours)
    const savedCurrentIndex = localStorage.getItem("liege_currentIndex");
    const savedScore = localStorage.getItem("liege_score");
    const savedQuizEnabled = freshCircuitStart
        ? null
        : localStorage.getItem("liege_quizEnabled");
    const savedCompletedQuizQuestions = freshCircuitStart
        ? null
        : localStorage.getItem("liege_completedQuizQuestions");
    ...
    if (forceTarget) {
        ...
        const visitedPoints = safeJsonParse(localStorage.getItem('liege_visitedPoints'), []);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    nextButton.addEventListener('click', () => {
        ...
        const advanceAfterQuiz = () => advanceToNextPoint();

        if (localStorage.getItem("liege_quizEnabled") === null) {
            showQuizPrompt((wantsQuiz) => {
                quizEnabled = wantsQuiz;
                if (quizEnabled) {
                    showQuizForPointIndex(leavingIndex, advanceAfterQuiz, { force: true });
                } else {
                    advanceAfterQuiz();
                }
            });
        } else {
            quizEnabled = localStorage.getItem("liege_quizEnabled") === "true";
            if (quizEnabled) {
                showQuizForPointIndex(leavingIndex, advanceAfterQuiz, { force: true });
            } else {
                advanceAfterQuiz();
    ...
    audioBtn.addEventListener('click', () => {
        ...
        if(current && current.audio) {
            ...
            if (quizResumeAwaitingAudio && quizEnabled) {
                ...
                quizPendingAdvanceCallback = null;
                setTimeout(() => {
                    showQuizForPointIndex(currentIndex, pendingAdvance || (() => {}), { force: true });
                }, 300);
            }
        }
    });
...
function getQuizArtifactsCacheVersion() {
  const src = document.querySelector('script[src*="quizData.js"]')?.getAttribute('src') || '';
...
async function loadQuizTranslations() {
    try {
        const version = getQuizArtifactsCacheVersion();
        const url = version
          ? `translations/quiz_translations.json?v=${version}`
          : 'translations/quiz_translations.json';
